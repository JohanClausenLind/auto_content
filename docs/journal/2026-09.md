...
## 2026-09-11 — the output lands on the node that made it, and a vision model looks at the set

The workspace could be run and could not be reviewed. The screenshot this session started from is
22 nodes at 20% zoom with a Run button, a Check button, and no way to see anything any of those
nodes had produced. A run's files were reachable — in a panel *over* the graph, grouped by file
type — and nothing connected a bad drawing to the node that drew it. ComfyUI answers that by
hanging every output off the node that emitted it, and the question is the same here: *which step
made this, and what else did that step make?*

### Which node made which file

No stage executor knows its own file list. `StageOutput` is a hash and a dict of facts, and forty
executors were never going to start agreeing about how to report one. So `runners/attribution.py`
**observes** instead: snapshot the run directory, run the step, snapshot again, and the files that
appeared or changed are that node's output. Measured before it was wired in, because a per-step
directory walk sounds expensive and the whole design rests on it not being — **20.8 ms for the
12,019 files of `m04-picture-story-24`**, against stages that cost 48 s to 608 s. A 24-step lane
pays half a second in total.

The cap keeps media before bookkeeping, and that ordering is load-bearing rather than cosmetic:
one `generate_keyframes` step on a 24-beat story touches 2,904 files, and a plain alphabetical cut
spends all 200 slots on `audio/` markers and records not one of the frames the step exists to make.

Two files had to be excluded by path and not by name. `run.json` is rewritten after *every* step,
so the walk handed it to whichever step ran last; `controls/<shot>/run.json` is a real output of
the control compiler and keeps belonging to it. `run.json` and `pins.json` also joined
`_MARKER_NAMES`, so the run's own paperwork stopped appearing in the review panel next to the film.

### Reading it back for the 241 runs that predate it

`services/run_nodes.py` is the reader, and it always says which of two sources it got: **recorded**
(the run observed it) or **inferred** (a path table guessed). Three findings from running it over
the real output tree:

- **The node universe has to be the lane, not the run.** `ps1c-pinecone` was resumed at
  `upscale_video`, so its twelve stage records begin there — and a node list built from them has no
  `anchor`, which left every anchor drawing in the run belonging to nobody. That is exactly the run
  somebody opens, because it is the one that needed a resume. Reading the lane from
  `workflows/catalog.stage_order` took it from **265 unattributed of 265 to 30**.
- **The lane has to be passed in.** Only 54 of 241 reports name their own workflow;
  `run_history` already resolves the rest through `durations.infer_workflow`, and `run_nodes` was
  reading `report["workflow"]` and getting `None`.
- **A path table needs candidates, not an answer.** `exports/` is a film on `narrated-video`, a
  card set on `single-clip-post` and a flipbook on `image-set`. Each key names the stages that
  could have written there, resolved against the lane's own nodes; two candidates present means
  the file stays unattributed. Coverage over the first 60 runs: **88% attributed, 12% honestly
  unplaced**, most of it in the 16 runs whose lane cannot be identified at all. The last four
  points came from the end-to-end run below, which caught the table pointing at the wrong node.

A run also now carries a **subject** — what it was rendering, in the operator's own words, from the
brief widget the report records or the story plan's own subject. 216 of 226 runs have one. Without
it the history is a list of directory names: `a20-imageset-owl` says which lane ran and nothing
about what came out of it, and "the iceberg one from last night" is not something you can find.

### The join to the canvas

`WorkspaceNode.key` is new: the lane's own name for a step (`anchor`, `spokes`, `frames_gate`),
carried through the template exporter into the graph document. Without it, matching a run to a
canvas could only be done on stage type — ambiguous in every lane that runs one stage twice, which
are the lanes where the answer matters most. `workspace/nodeOutputs.ts` does the join and refuses
to guess: a hand-built node with no key gets nothing, two nodes claiming one key get nothing and
the step is reported as unmatched, and the steps of the run that are not on this canvas are named
rather than counted — that sentence is what explains a graph which otherwise looks like the run
produced nothing.

Picking a run lays it over the canvas and opens its pane; **closing the pane leaves the drawings on
the nodes**, which is the view the whole thing exists for. A panel in front of the node page cannot
be the answer to "show me the output on the nodes".

### A vision model on the set

`qc/frame_review.py` and `qc/reviewer.py` both already said, in almost the same words, what the
measurements cannot do: *whether the subject is the same subject. Two drawings of different objects
in the same palette and light score as consistent, which is precisely the failure this machine
produced when "honey-coloured" drew jars of honey.* `ReviewerKind` has been
`operator | agent | vlm` since the contract was written. `qc/vlm_review.py` is the `vlm`.

It sends the story, each frame's recorded intent, and **every frame in one call**, because the
question that matters is about the set and a model shown one picture at a time cannot answer it.
The catalogue's quality tier is the only entry declaring `vision` (`ollama show` reports a clip
projector on that build), and `required_models` pins it, so a text-only model can never be handed
an image list and answer confidently about pictures it never received.

Two measurements shaped the call. The first attempt was **refused**: six frames at their native
2560×1440 came to **22,494 prompt tokens** against a 16,384 window. Measured per frame against the
running Ollama:
...
768 buys a twelve-frame review for about 4,200 tokens, and is more than enough for the question —
`qc.frame_review` measures tone, colour and palette distance at 640×360.

**It is an opinion and never a verdict.** Nothing in it can accept or reject a frame; the review is
stored beside the batch, bound to the digests of the pictures it saw, and the operator still presses
their own button with their own reason. "Mark these" fills in their selection and will not overwrite
a decision they already made on the same picture. `shows` — what the model says is *in* the
picture — is asked for first and shown first, because reading it against the thumbnail is the only
check there is on the reviewer: a model describing the brief back has told you nothing.

It does not take the card from a run either. The vision tier is 17.8 GB on a 24 GB card, so a review
asked for mid-run is refused with the run named rather than evicting the weights that run is using —
the mirror of the check `_release_gpu_if_idle` already makes — and `keep_alive=0` hands the card back
the moment the answer arrives.

**What it found on `w-iceberg`**, a real set of six parked at the gate. `drift_qc` could only say the
set had no consistent core and named no frame (median 0.243, and naming all six outliers says
nothing). The vision model named **`frame:0003`** and said why: a higher camera angle, a smaller
iceberg, and a water pattern of regular concentric circles the other five do not have. It also
caught something nothing else had — every frame has background mountains and glaciers, and the
brief says "nothing else on the water". 42.7 s, 2,910 input tokens.

### Proving it on a real lane

`image-set` was run for real into `output/local-runs/attribution-check` — `generate_anchor` took
**47.8 s** and left a 2.4 MB PNG, so this is the real backend and not the mock. Read back through
the history afterwards:

```
workflow: image-set | outcome: stopped
subject: a wet river pebble, banded grey and rust        <- what the operator typed
outputs: 26 / 26 | unattributed: 0

  story        plan_story                   ran=True  recorded  files=  2
  anchor       generate_anchor              ran=True  recorded  files=  3
  lock         lock_generation              ran=True  recorded  files=  1
  controls     compile_controls             ran=True  recorded  files= 18
  spokes       generate_keyframes           ran=False inferred  files=  0
  ... the rest of the lane, present and not run
```

It also caught a real fault, which is the reason it was worth running. `generate_keyframes` was
stopped part way (the card was busy) and a **stopped** step appends no stage record, so the one
file it had written — `sequence/anchor.png`, the hub every spoke is edited from — fell through to
the path table, which attributed it to `package_sequence`: a stage that had not run at all. The
table now names `generate_keyframes` and `lock_generation` as its two candidates, so on a lane
holding both it goes **unattributed** instead of pointing at the wrong node. An ambiguous answer
being worse than no answer is the whole rule, and it took a real run to find the place the code
was breaking it.
...
```
just lint                                                      -> exit 0
just typecheck                                                 -> exit 0 (pyright + 9 tsc projects)
just test                                                      -> exit 0 (pytest + 10 vitest
                                                                  projects + the app build)
uv run pytest -q                                               -> 1479 passed, 1 skipped
                                                                  (1455 -> 1479: +14 attribution,
                                                                  +10 vlm review)
pnpm -r test                                                   -> 463 passed
                                                                  (node-graph 58->68, web 134->162)
uv run pytest tests/unit/test_run_node_outputs.py -q           -> 14 passed
uv run pytest tests/unit/test_vlm_review.py -q                 -> 10 passed (the real gateway
                                                                  through its `completion_fn`
                                                                  seam, so routing really
                                                                  resolves the vision weights)
uv run pytest tests/api/test_ai_review_api.py -q -m integration -> 7 passed
content-factory run-local image-set --project-dir ...          -> anchor 47.8 s, 26 files,
                                                                  0 unattributed, all recorded
content-factory frames ai-review output/overnight/w-iceberg    -> named frame:0003, 42.7 s
just schemas                                                   -> WorkspaceGraph gains `key`
```

**Not done.** A step **stopped mid-execution** records no outputs, because the stop path appends no
stage record at all; its partial files fall back to the path table. That is what the run above hit,
and the table was corrected rather than the stop path, because changing what a stopped run's report
contains touches `_outcome` and the duration medians. The `awaiting_review` badge is attributed to
a lane's gate only when it has exactly one; a lane with two gates gets no badge, because the count
is per deliverable and deliverables are not keyed by node. `MAX_FRAMES` is 12, so a larger set is
reviewed as its first twelve with the count said plainly rather than sliced into several calls. And
the review still cannot be asked for while a run holds the GPU, which is the right refusal and not
a nice one.

---

## 2026-09-12 — why it doesn't look real, and the 39 GB nothing was reading

Started as one question from the operator — *why doesn't the content look real, it looks low-poly*
— and the answer turned out to have two halves that had never been connected: the pictures are
missing the evidence of a camera, and the data assembled to give them that evidence was not
plugged in.

### What the pictures actually show

Six sets opened at full size and at 1:1, plus the control images being fed in. Texture measured as
the fraction of 16-px tiles whose high-pass residual has a standard deviation under 0.6:

```
demo-pebble   anchor, `single-image` lane        95.4 % dead-flat   median tile detail 0.12
w-iceberg     keyframe                           91.4 %                                0.09
ps2b-amber    anchor                             86.7 %                                0.11
ps2b-amber    after SeedVR2                      75.6 %                                0.46
--------------------------------------------------------------------------------------------
demo-pebble   keyframe 0, two of four frames      0.7 %                               35.14
```

A photograph runs under 10 %. That single number is most of what "it doesn't look real" means
before composition or anatomy come into it: a sensor puts a noise floor on every pixel including a
blurred sky, and none of these have one anywhere.

The subject is *literally* low-poly, and it happens with nothing attached. `w-iceberg`'s **anchor**
— `references: 0`, no control image, just the `photographic` preset — is already a triangulated
flat-shaded mesh on procedural water whose ripple wavelength is constant from foreground to
horizon. `ps2b-amber` is a faceted gem in four of six frames; `ps2c-amber`, the same brief, is a
hexagonal honeycomb blob; `demo-pebble` is a ceramic paperweight with painted decal banding, for
"one wet river pebble, banded grey and rust, on plain pale sand". In all twelve amber frames the
insect is the same flat black silhouette, centred, **not refracted by the resin it is inside**.

And six "shots" are one plate: in both amber sheets the ground is pixel-identical across six frames
that asked for different cameras, with zero parallax. Same in the iceberg — identical background
mountains, only the berg scaling.

### The red rectangle

`run_sequence` compiles exactly one control kind, `layout_boxes`, and both reference backends
appended that raster to the model's reference list. The raster is two colours:

```
(0,   0, 0)  1,762,560 px   85%
(255, 0, 0)    311,040 px   15%
```

A pure `#FF0000` box on black, sent as a *reference image* to a pipeline whose own settings
docstring says it "treats every reference as subject material". Measured on `w-iceberg`, anchor
against its own six keyframes:
...
Every frame that received the box went warm and lost a tenth of its exposure — the flat magenta
veil over that set, which the anchor does not have. It is also a scheduler switch, the same trap
`_conditioning_for_frame` already documents for a skeleton of nobody: appending the raster changes
`len(ref_images)`, and upstream branches the whole dev recipe on `== 1`.

`image_sequences.control_as_reference` is now **False** and both backends honour it. The flag rides
in the frame marker hash, so flipping it regenerates rather than serving the red-boxed frame for
ever.

### The presets were asking for a render

`photographic` was nine clauses of which four were negations — "no outlines, no posterisation, no
illustration, not a drawing" — on weights distilled to guidance 0, where this file's own comment
already establishes that a negative has no classifier-free guidance to act through. The realistic
presets now say what a photograph *has*: grain, halation where a highlight clips, dust and wear.
That is the better instruction at guidance 0 whether or not the negations were also doing harm,
and it does not depend on guessing what the text encoder does with "not".

`shallow depth of field` left `photographic` — it was house style on the preset every realistic
lane uses, and `prompt_compile` already records it wrecking `ps1-pinecone`. It stays in `cinematic`,
where it is the point of the look.

`image-set` carried its own hand-written style: *"consistent character and material design, even
flat lighting, no dramatic shadow, plain neutral background, same rendering idiom in every image"*.
That is a specification for a 3D asset turntable and it returned one. It is now the
`photographic_set` preset — a medium plus the things that must hold still between images. Three
other lanes moved onto named presets; a written-out style has no name and therefore no per-style
drift override, which `style_name_for` reports honestly as `""` and nobody had noticed.

**Correction to an earlier reading in the same session:** `style/editorial.py`'s
`IMAGE_STYLE_SUFFIX` ("technical editorial illustration … clean vector-like silhouettes") is **not**
reaching any still. `image_prompt` has no callers. The pebble's painted look is the hand-written
lane style above, not that suffix.

### The 39 GB

`content-factory datasets list` now answers what took grepping the facts of 236 runs:

| library | size | reachable? |
|---|---|---|
| sound libraries -> `assets/sfx` | 12 GB -> 940 MB | **yes**, 670 curated sounds, live |
| human-interaction reference | 20 GB, 15,789 clips | wired, never returned a usable match |
| Blender staging assets | 614 MB | 3 of 236 runs, mannequin only |
| romsketch LoRA + training set | 6 GB | **no** — no HiDream LoRA path existed |
| `assets/music` | 113 MB, 22 beds | **no** — `music_dir` pointed elsewhere |
| InterGen / Inter-X | 53 MB | code only, motion data never fetched |

**The reference library was working.** All three runs that ever called `find_reference` recorded
`selected: 0`, and all three were a Rayleigh-scattering explainer — *"Sunlight is white. It carries
every colour at once."* A two-person interaction library correctly has nothing for that. Asked a
question it can answer, it answers instantly:
...
The last line is the other half: only 58 of 15,789 indexed clips are retargeted, so "matched well"
and "can be staged" come apart constantly, and **both outcomes reported as `selected: 0`** —
indistinguishable from an absent library. `selection.json` now carries `stageable_beats` and a
`reason` that reads differently in each of the three cases.

`music_dir` pointed at `fixtures/music`, which holds one four-second synthesised sine chord — which
is why all 29 runs that ever selected music selected `calm_bed_a`. It was the only track there was.
`scripts/build_music_index.py` writes the `tracks.json` `assets/music` never had.

### Commands run

```
just fmt                                             -> 5 files reformatted, 1 ruff fix
just lint                                            -> exit 0 (ruff + oxlint; warnings only)
just typecheck                                       -> exit 0 (pyright + 9 tsc projects)
just schemas                                         -> 70 schemas, 60 node types, 16 templates;
                                                        idempotent on a second run
uv run pytest -q                                     -> 1510 passed, 2 skipped
                                                        (1479 -> 1501: +7 film response,
                                                         +4 texture QC, +3 reference staging,
                                                         +3 hidream lora, +3 lora spawn,
                                                         +2 style presets, +9 beat gaps)
pnpm -r test                                         -> 463 passed, 0 failed (9 projects)
pnpm -r run build                                    -> exit 0 (the third leg of `just test`)
just test-integration                                -> 68 passed (was 2 failed, then 1; both
                                                        causes fixed, see below)
uv run content-factory datasets link                 -> 13 linked, then 13 ok (idempotent)
uv run content-factory datasets check                -> 13 declared, 13 present, 0 unread
uv run python scripts/build_music_index.py           -> 22 tracks indexed
uv run content-factory frames film <iceberg frame>   -> dead-flat 91.4% -> 0.0%
```

### The run-stop test was not flaky. It was wrong, and retries were hiding it

`just test-integration` kept failing on
`test_run_stop.py::test_stopping_a_producing_run_cancels_the_node_in_flight`, and it passed in
isolation every time. The obvious reading — and the one written in my own notes from an earlier
session — is interference from the parallel session in this checkout. That reading was wrong, and
it survived because nobody had read the assertion:

```
AssertionError: run reached FAILED: activity Heartbeat timeout
```

The run died in **preflight**, before the state the test is about. `execute_node` took the injected
test delay as a bare `await asyncio.sleep(delay)` *before* the heartbeat loop started:

```python
delay = float(os.environ.get("CF_TEST_STAGE_DELAY_S", "0") or 0)
if delay:
    await asyncio.sleep(delay)  # 8 s, and HEARTBEAT_TIMEOUT_S is 6
task = asyncio.create_task(...)
while not task.done():
    activity.heartbeat(inp.node_id)  # first heartbeat, 8 s late
```

The test sets that variable to 8 against a 6-second heartbeat timeout, so **every activity in that
run missed its first heartbeat**. It passed whenever a retry happened to land inside the window —
the failing logs show `attempt: 2` — which is exactly what a load-sensitive flake looks like from
the outside. The delay exists to make a node slow enough to catch mid-flight, and a slow node is
the thing heartbeating is for, so the delay now heartbeats through itself. Integration went from
1 failed to **68 passed**.

**This closes finding 107.** The journal has called this test flaky since 2026-09-10 and blamed it
on box load three separate times (lines ~7205, ~7238, ~7325, ~7553 above). Load was a real
correlate — under load a retry is less likely to land inside the window — but it was never the
cause, and "flaky, confirmed over three runs" is what a real defect looks like when the assertion
is never read. Every one of those entries, and a note in my own memory, repeated the diagnosis
instead of checking it.

Worth keeping: "it passes alone, so it is interference" was a plausible story that matched what was
already written down, and it was wrong. The assertion text settled it in one line.

### One failure that was not mine, fixed anyway

`just test-integration` came back with two failures in `tests/api/test_graphs.py`, both of the form
"round-tripped document has a field the expected dict does not". The field is `key` on
`WorkspaceGraphNode`, added by the parallel n8n-pattern session in this same checkout; the schema
gained the field and this test was not updated, so the shared tree was red before this session
touched it. Fixed here because the gate had to be green and the fix is mechanical — the test
already had a helper-shaped comment for exactly this ("a document written before folded groups
existed comes back with an empty list"), so `_with_server_defaults` now names the pattern and the
next defaulted field is one line rather than two silent round-trip failures.

### The voices were cut off, and it was measurable

Reported as "the voices cut off at the end instead of being smooth when they start talking at the
next timestamp". `beat_spans` ran each beat from its first word's `start_ms` to its last word's
`end_ms` — both ASR word edges — so two things happened at every boundary. The pause between
sentences was **discarded**, and the clip was truncated where the transcriber stopped counting
rather than where the sound stopped.

On `ps2b-amber`, six beats and five gaps:

```
beat                start      end      len   gap to next
beat_000000001          0     6400     6400   1000
beat_000000002       7400    14520     7120    520
beat_000000003      15040    20680     5640    900
beat_000000004      21580    28720     7140   1140
beat_000000005      29860    34400     4540    680
beat_000000006      35080    41380     6300      -
                                     discarded: 4.24 s
```

And the cuts landed on speech, every one — RMS of the last 20 ms of each clip, where silence sits
below −60 dBFS:

```
beat 1  −20.7 dBFS      beat 4  −28.1 dBFS
beat 2  −37.9 dBFS      beat 5  −22.2 dBFS
beat 3  −28.3 dBFS      beat 6  −28.1 dBFS
```

Beat 1 was chopped at −20.7, which is close to full speaking level. The heads were the same story
in reverse: beats 2–5 began at 0.06–0.09 RMS, so the next line started already at volume.

A boundary now falls *inside* the gap instead of on a word edge: the next beat takes at most
`HEAD_LEAD_MS` (220 ms) of run-up and the remainder stays as the previous beat's tail, so a long
pause does not front-load onto the next line. The first beat gets a lead-in, the last gets 400 ms
of decay clamped to the recording.

The proof is not the edge levels, it is losslessness. Re-cutting the real recording with the new
spans and concatenating:

```
new concat   41.78 s   byte-identical to the source span: True
old concat   37.14 s   byte-identical to the source span: False   (−4.64 s)
source       42.11 s
```

The beats now abut sample-for-sample, so the mix reconstructs the original performance exactly and
there is no discontinuity at any boundary to hear — whatever the level there. One boundary
(beat 4 → 5) actually reads *louder* than before at −23 dBFS, and it does not matter for precisely
that reason: the waveform continues rather than jumping. `test_beat_gaps.py` pins the property on
a deterministic ramp, where a dropped or repeated sample cannot hide in silence.

### Rendered: three models on the same three prompts

The first GPU work since the prompt changes. Same prompt text, same seed 42, three backends.

| | recipe | wall | dead-flat tiles |
|---|---|---|---|
| HiDream-O1 **dev** (skill server) | 28 steps, guidance 0 | ~36 s | 50.4 / 57.0 / 64.2 % |
| HiDream-O1 **full + romsketch** (musubi) | 50 steps, guidance 5, 24 blocks swapped | ~15 min | 6.4 / 15.2 / 21.3 % |
| **FLUX.2-dev** turbo (ComfyUI) | 8 steps | ~2 min | **0.1 / 0.2 / 6.9 %** |

Three findings, in order of how much they change what to do next.

**FLUX.2-dev is the realism answer and it is not close.** Two orders of magnitude more texture than
HiDream dev at a fifteenth of the LoRA's wall clock, and it looks it — moodier light, real bokeh,
filmic grain, less stock-photograph. Its 8-step turbo recipe does cost anatomy: one of the three
merged two figures at the embrace. It belongs on the realistic lanes.

**romsketch is a STYLE adapter, not a realism one.** The captions say "two-person affection" and I
read that as content; rendering it settled what it actually is — pen-and-ink line with watercolour
wash, on paper, with a visible drawn border. "rom" + "sketch". It holds one idiom across three
unrelated prompts and draws joined hands correctly, which is the hard part. The settings docstring,
the registry note and docs/datasets.md all said the wrong thing about it until this run and now do
not. Its trigger is `romsketch style.` at the head of the prompt.

**HiDream dev is capable of photographs.** The same weights and the same server that produced a
faceted low-poly amber gem produced a wet railway platform with a real train, background crowd and
believable coats. What changed: no red raster, a subject that names a place, and a preset that asks
for grain rather than denying illustration.

### And the amber still fails, which is the honest half

Three attempts at the same brief through the full lane, each reviewed frame by frame:

1. `amber-refix` — the original subject. Faceting **gone** (smooth spheres, not low-poly gems),
   but two of six had an open circular rim: hollow glass vessels, not a solid. Rejected 3 of 6.
2. `amber-forest` — subject rewritten to name a place. The forest floor came back genuinely
   photographic, and the subject became an opaque dough-coloured lump with the insect sitting on
   top of it. Rejected 6 of 6.
3. `amber-lit` — translucency and backlight named explicitly. Correct object at last: deep orange,
   glowing, insect suspended *inside*, all six agreeing. **The faceting came back with it** — a
   low-poly dome in every frame.

So the faceting is this model's signature on a translucent irregular solid and it did not move with
the prompt, in either direction. The dead-flat fraction barely moved across all three either
(87–90 % → 85–88 % → 84 %), which corrects a hypothesis from earlier in this same session: the
couple scenes measured 50–64 % because they are full of small objects at distance, not because
their prompts were phrased better. A macro shot of one smooth object is inherently low-detail, and
this recipe adds no grain to it. For this subject the levers are the film pass and FLUX.2, not
words.

One workflow gap found doing it: a **rejected frame never regenerates**. `generate_anchor` keyed
6/6 cache hits on the re-run, so the verdict blocks the lane for ever unless the prompt changes and
moves the input hash. `prompting propose` is presumably meant to close that loop; nothing does it
automatically.

### Rendered: two films

```
videos/romsketch-lora-clip.mp4     16 s   1080x1080   the LoRA set, held cut, film pass
videos/amber-lit.mp4               44 s   576x1024    the full lane, captions burned in
```

The second one carries the audio fix, and its own facts say so against the run it replaces:

```
                    spoken   unused    mix      music track
ps2b-amber (before)  37.1 s   5.0 s   39.89 s   calm_bed_a
amber-lit  (after)   41.8 s   0.3 s   44.53 s   doc_strings_reflective
```

`unused_seconds` 5.0 → 0.3 is the discarded breath coming back, and the music track is the second
fix landing: the bed library has 22 beds and every previous run picked `calm_bed_a` because
`fixtures/music` held exactly one file. Cuts landing on speech went **6/6 → 1/6**, and the one that
remains sits inside a breath where the clips abut byte-exactly, so there is nothing to hear.

### Ideogram 4, and the mistake that nearly buried it

New weights appeared at `/mnt/fast/models/ideogram-4-comfy` (28 GB) mid-session. Researched, and
the full write-up is `docs/research/2026-09-12-ideogram-4.md`; the recipe was transcribed from
ComfyUI's own shipped blueprint node by node and link by link rather than guessed.

**The first seventeen generations were 94 % refusals.** A flat grey card with "Image blocked by
safety filter" drawn across it, or — worse — a plausible photograph with the refusal lettered over
the top, sometimes garbled: "Image blocked by safery filter" over an amber macro, "Imargia
afforraberise paricitanaiton" over a perfectly good pine cone. A brass orrery refused. A pine cone
refused. A fishmonger's sign refused.

Three things had to be ruled out before blaming the model, and ruling them out is what found the
answer:

* **Was the graph wrong?** No — verified against the blueprint's links, including which UNET feeds
  the guider's negative side.
* **Was it content?** No — the same pine cone refused on seeds 42 and 12345 and rendered on seed 7,
  and both blocked seeds blocked again at 48 steps as well as at 12. A content filter is
  deterministic on content.
* **Was a file missing?** No — the tutorial's second encoder is not used by this CLIP type.

The answer was in `comfy/text_encoders/ideogram4.py`: the tokenizer wraps the prompt in a chat
template, and ComfyUI's own comment says the encoder does "multimodal **generate**" as well as
conditioning. **A prose prompt is a chat turn.** The Qwen3-VL assistant answers it, and when it
answers with a refusal that refusal becomes the conditioning, which the DiT faithfully letters onto
the canvas. The typos are the DiT drawing text; the seed dependence is the sampling of that
generate step.

The model is trained on structured JSON captions. Fed one, the same pine cone that refused 3 of 3
as prose came back **3 of 3 clean**. I was using it wrong, and the 94 % was mine.

```
                        prose                JSON
pine cone, 3 seeds      0/3 clean            3/3 clean
3 comparison prompts    0/3 first try,       2/3 first try,
                        5 seeds each         third on its second seed
```

With JSON it is the best of the four on texture — 0.2–0.3 % dead-flat tiles against FLUX.2's
0.1–6.9 % and HiDream dev's 50–64 % — and the most filmic light. Two caveats from three frames: it
drew gibberish lettering and a photo-viewer overlay into one, and put three people on a bench where
the caption said two.

### A detector for a model that answers in pixels

`qc.frame_review.is_refusal_frame` catches both modes, because the second one is the one that would
otherwise ship. The flat card is trivial (median tile detail 0.00 against 0.4–2.3). The banner is
structural — a burst of horizontal edge energy confined to a few rows in the vertical centre — and
is deliberately **advisory**, because the margin is thin:

```
refusal over a photo              16.35 x the median row
refusal, garbled lettering         5.71
refusal over the amber macro      44.04
-------------------------------------------  threshold 5.0
a lit train at the frame centre    3.93
a night street                     3.68
clean Ideogram / FLUX.2 / LoRA     1.7-1.9
```

It is 12 for 12 on the frames measured here — including one I had already called clean by eye and
was wrong about. That one mattered: I had scored `seed 7` as a success on detail alone, so my first
refusal rate was an undercount, and the detector caught my error rather than the other way round.

### A rejected anchor now redraws — and the fix had the same bug twice

Reported from the amber runs: reject three frames, resume, and `generate_anchor` reports
`cache_hits: 6`. The verdict the gate exists to collect was the one thing the run could not act on.

The keyframe lanes solved this on 2026-09-10 — `_clear_rejected_frames` moves the picture and its
marker into `sequence/rejected/`, and `_rejection_seed_offsets` counts those files to walk the
seed. The anchor lanes had nothing, so the fix is that pattern mirrored: `_clear_rejected_anchors`
and `_anchor_rejection_offsets`, with `_generate_checked` taking a `seed_offset` that moves the
whole attempt ladder.

The **×8** multiplier is not arbitrary and I would have got it wrong: the three blocker retries
inside one run already walk the seed by 0, 1, 2, so an offset of one lands the redraw on the second
attempt of the run that was rejected — a picture the reviewer has in effect already turned down.

Proven on the stuck run itself:

```
                          verdict        was         now    seed  changed
shot_3edc6d21eb2a:0000    accept   2b1a27d0a0  2b1a27d0a0      7  no
shot_9ff49b91f418:0000    accept   4216027769  4216027769      7  no
shot_b0477c619ed0:0000    accept   66eb203ded  66eb203ded      7  no
shot_ad71f1193c34:0000    reject   78b9ad1294  c6b1584bc4     15  YES
shot_d3f50497205c:0000    reject   5f46d9e433  4a9d9954d9     15  YES
shot_fb17318f4180:0000    reject   81c5f7d3ac  d96c273b59     15  YES
```

`cache_hits` went 6 → 3, the three turned-down pictures are kept in `anchors/rejected/` as the
evidence for the reason, and two of the three redraws fixed the defect they were rejected for (the
open rim is gone; they are solid spheres). The third went the other way and came back a corked
bottle — which is what a blind reseed does. It redraws; it does not act on the reason. Closing that
loop is `prompting propose`'s job and is still not automatic.

**Then the second resume redrew nothing.** `cache_hits: 6` again. `_clear_rejected_anchors` was
reading `batch.json`, which is what the *gate* wrote when it last ran — and the operator's answer
goes to `verdict.json` afterwards. Reading only the first works once, because the gate had already
merged the previous answer, and then silently stops. Both clear functions now read
`frame_reviews.current_batch`, the merged state the panel and the CLI already share. The keyframe
path had the identical latent flaw and had simply never been exercised twice in a row.

Third resume: `cache_hits: 5`, one frame redrawn, the one that was rejected.

### The other half: a redraw must not cost a re-review of the whole set

Making rejection work exposed a second gap immediately. `missing_decisions` required an agent to
name *every* frame, and `decide` reset any frame it was not told about to `unreviewed` — so
rejecting one drawing meant re-reviewing all six, which is the opposite of what `image-set`
promises ("rejecting one drawing costs one drawing, not the film"). Nobody could have noticed
before, because until this session the second review never happened.

A frame carrying a verdict about a byte-identical picture is decided: `merge_verdict` only carries
one when the digest is unchanged. So `decide` keeps an unnamed frame's existing verdict and reason,
`missing_decisions` asks only about `unreviewed` ones, and the review request lists only those. A
fresh batch is unaffected — everything in it is `unreviewed`, so every frame must still be named.
Verified live: naming three frames of six produced `accepted: 5, rejected: 1`.

While in there: the single-anchor branch records `shot_id: None` and `review_frames` did
`.get("shot_id", "anchor")`, which returns `None` when the key is present — so that lane's frames
were called **`None:0000`** in every review request and verdict. Fixed at the source;
`_anchor_frame_paths` still accepts the old spelling so a verdict already on disk keeps working.

### The loop closed: what the reviewer says now reaches the model

The gap left by the redraw fix was that a redraw moved the **seed**, not the **prompt** — it came
back different, not necessarily better, and one of three came back a corked bottle.

First, a correction to what I wrote earlier in this same entry: **`prompting propose` was never
going to close this.** Reading it: it learns from *measured* findings (`tonal_collapse`,
`monochrome_partial`, `edge_intrusion`, `continuity_break`), its failure→rule mapping is
deliberately fixed in code so it "cannot invent a rule it was not taught", and it proposes edits to
the global guidance file. It cannot act on free text like "an open circular rim: a hollow vessel".
Different loop, still useful, not this one.

What was actually missing is that a rejection carried only a complaint. `reason` is a complaint and
cannot be sent to an image model as one — `sequences.styles` already measured why: at guidance 0 a
negation is a hint the model may or may not take, and "what *does* work is a positive instruction
about the scene". So `FrameRecord` gains a second field:

```
reason    "A corked glass bottle holding liquid with an insect floating in it — a container,
           not a piece of amber."                            <- the record
redirect  "one solid opaque lump of hardened orange resin, the insect sealed inside the
           resin itself"                                     <- what the model is given
```

`redirect` is appended to that frame's prompt on the redraw, as a description rather than a
prohibition. Empty is allowed and means seed-only, which is the behaviour it replaces.

**It is sticky, and that was the subtle part.** A redirect read straight from the verdict vanishes
the moment the frame is redrawn — the new digest makes it `unreviewed` again — so the prompt would
revert, the input hash with it, and the frame would regenerate *without* the correction it had just
been given. Same failure shape as reading `batch.json` instead of the merged state, one layer down.
Each draw records the set it was made with, that marker moves aside on rejection, and the next draw
reads the most recent one and appends what is newly pending. Corrections accumulate.

Proven on the bottle:

```
cache_hits: 5          only the rejected frame redrew
seed: 31               7 + 3 rejections x 8
redirects: ['one solid opaque lump of hardened orange resin, ...']
prompt:    "... warm raking light, even studio light ... . one solid opaque lump of hardened
            orange resin, no bottle, no cork, no neck, the insect sealed inside the resin itself."
```

The bottle, cork and neck are gone; it came back a solid lump with the insect sealed inside.

Carried to every surface, not just the one I was testing: the CLI (`--redirect`), the API
(`VerdictBody.redirect`), the service, `merge_verdict` (or it is lost the moment the gate re-runs),
and the workspace panel, whose field says to describe the picture rather than the fault and says
why.

One more of the same bug, found on the way: **the CLI decided on raw `batch.json`**, not the merged
state that `record_verdict` and the panel use. Answering a redraw would have silently un-accepted
every frame that was not redrawn, because `decide` can only preserve a decision it can see.

### The mocap chain ran, and four guards fired on the way

Asked for a run of two people holding hands using everything unused. `scene-controlled-video`
with `--shots fixtures/shots/two_hander_mocap.json` — six shots, two characters each, posed from
real CMU takes through the `segments` retarget path. It worked, and getting there cost four
failures, every one of them a guard doing its job:

1. **fps mismatch.** The drafted story was 30 and the CMU clips are 24. The error explained that
   `compose_video` conforms by *duplicating frames*, "which is how every wind short v1-v5 shipped
   24 fps footage in a 30 fps film". Fixed by using `fixtures/story/love_story.json`, authored at
   24 with six beats against six shots.
2. **`review_assets`.** Three character rigs needed a person to look. Opened all three turnaround
   sheets — six views each, clean MakeHuman basemeshes, T-pose correct — and approved them.
3. **No `appearance`.** `compile_controls` refused: "the shot plan stages 12 figure(s) and none of
   them has an `appearance` … the image model draws exactly that — an untextured grey mannequin,
   measured over ten anchors". **This is why the three historical Blender runs produced mannequins**;
   the guard is newer than those runs. Fixed in `scripts/make_mocap_shot_plan.py` with an
   `APPEARANCE` map keyed by asset, so the same description holds across all six shots.
4. **The wrong Blender.** `OSError: not a blend file`. The file was fine — zstd, decompressing to a
   valid `BLENDER17-01v050` header. `/usr/bin/blender` is an **apt 4.0.2 from April 2024** and it
   shadows the snap's 5.2.1 on PATH; the assets were built with 5.x. `controls.blender_bin`
   defaults to `"blender"`, so `shutil.which` had been picking the old one. `/snap/bin/blender`
   loads the same file and reports `collections: ['CH_man_01']`.

**This is the fourth thing the Blender path needed that nothing said out loud**, on top of the
three from earlier today. It is not that the path is broken — it is that four separate
preconditions were each silently unmet.

Then it ran, and every number below is a first for this repo:

```
plan_shots        staged_from: [cmu_18_19_01, ...]      non-empty, in any run, ever
compile_controls  20.6 s, blender, 6 shots, 7 pass kinds
generate_anchor   1434.5 s, 12 anchors (two per shot), conditioned on pose_skeleton + depth
generate_video    436.3 s, LTX-2.5 along the staged motion
passed: true      8 stages, 1891.7 s  ->  videos/hands.mp4
```

The `hold_hands_walk` shot (cmu_22_23_08, 46 frames, two actors, contact: hands) went clay figures
→ two OpenPose skeletons → **a photograph of two people mid-stride on a street, hands joined**,
matching the captured stride. Clothed, correct anatomy, not a mannequin.

Honest about the rest of it: the **anchors** are good and the **motion between them is not**. At
19 s two figures overlap into a tangle of arms and legs — the contact failure the mocap exists to
prevent, happening in LTX-2.5's interpolation rather than in the staging.

### Four models on the same scene

| | dead-flat | detail | wall |
|---|---|---|---|
| HiDream dev **+ mocap skeleton** | 47.7 % | 0.77 | ~120 s |
| HiDream dev, text only | 67.9 % | 0.28 | ~34 s |
| FLUX.2-dev turbo | 22.1 % | 1.31 | ~84 s |
| **Ideogram 4**, JSON + layout box | **0.8 %** | **3.03** | ~48 s |
| HiDream full + romsketch | 42.7 % | 0.63 | ~6 min |

Ideogram is far ahead on texture again and looks it — real brick, real cobbles, real light — and it
**drew three people where the caption and the bounding box said two**. FLUX.2 read "oval highlight
bokeh" from the `cinematic` preset as a literal oval iris and masked the frame; that is my rewritten
preset biting on a model it was not calibrated against. The mocap-conditioned HiDream is the only
one of the five whose *pose* is what was asked for.

**A correction:** the romsketch LoRA did **not** force sketch here — it returned the best
photographic frame of the five, correct subject count, moody street. Earlier today I wrote that it
"pulls hard toward that idiom" even without its trigger. With a strongly photographic preamble
leading the prompt at guidance 5, it does not. The adapter is weaker than I claimed.

### Captions, restyled

The operator asked for no dark background, more movement, and Helvetica Neue Condensed everywhere.

`caption_box_alpha` was 0.55 — a translucent slab behind every line, and the reason burn-ins read as
a subtitle track laid over the picture rather than part of it. It is 0.0: an outline and a soft
shadow carry the type instead, and the parameter stays for a film graded so flat that an outline
disappears into it. `caption_pop` (0.10) grows the word being spoken and settles it back over
190 ms; `caption_fade_ms` (90) fades each cue's first and last word. **This overrides a recorded
house rule** — "flat colours, no gradients/glow/bounce", 2026-09-10 — at the operator's request, and
`pop=0, fade_ms=0` restores it exactly.

Helvetica Neue is licensed, so it is **not** vendored: `docs/licensing.md` bars redistributing
licensed media and a repo is redistribution. It is named in the settings and resolved through
fontconfig from the operator's own install, with Inter and Sora still pinned behind it in
`FONT_STACK` — so a machine without it renders in Inter rather than rendering in nothing. The SRT
burn path was given the same treatment, so the two caption paths are one house style.

### Three tests that were never portable

Deleting the runs exposed them; they had been passing on machine state.

* `test_anchor_conditioning` read `output/overnight/ps1-pinecone/.../bundle.json` — a *git-ignored
  run output*. It now builds the bundle from `sample_motion_plan` through the real compiler, which
  is the stronger assertion: the builtin fixture is **still** two hands today, rather than was once.
* `test_local_runner` asserted `logs[0] == "==> plan_shots"`. That is a claim about the forecast,
  and `services.durations` globs `output/` — so it held only while that directory happened to carry
  timings for both stages, and broke twice in one day: once when the runs were deleted, once when a
  new run timed one stage but not the other.
* `test_captions_per_beat` split on a literal `\1c<colour>}`, which the `\t` scale tag now sits
  inside. Matching on the exact string would have asserted the animation away.

### Not done, and why

**The film response is not wired into any lane.** `content_factory.imaging.film` is written, tested
and exposed as `content-factory frames film`; it moves the iceberg keyframe from 91.4 % dead-flat to
0.0 % and median tile detail 0.09 -> 2.58. It is deliberately not automatic. Looking at the result:
the grain is right and the iceberg is still a faceted mesh, so the frame now reads as *a photograph
of a polystyrene model*. That is better and it is not the fix — the prompt and the model decide
whether the picture is of a photographable thing. Turning it on before a GPU run measures the new
presets would bake a cosmetic over an unmeasured change.

**The romsketch LoRA cannot be used as configured.** The loader is written and the server refuses a
mismatch rather than merging quietly. The adapter was trained against the **full** weights and
`hidream_model_type` is `dev`; the module tree is identical, so every key would match and the merge
would succeed onto weights it was never fit against. Using it means moving the server to the full
weights, which is a GPU-time decision.

**`controls.compiler` is still `motion_plan`.** The Blender path works and produces the real passes,
but what those three runs staged was an unclothed default-A-pose mannequin in a grey void — no mocap
applied, no environment. Switching the default before the staging path has been exercised end to end
would trade a bad control image for a worse one.

**Nothing was re-rendered.** Every number above is measured on existing output or on synthetic
frames. The prompt changes invalidate every anchor hash by design and have not been run on the GPU.

### Fifteen more images: three sets, five models, one preset bug found

The operator asked for "3 sets more of images" — the same five lanes as the four-model table above,
on three new two-person scenes (a handshake, a hand on a shoulder, one sheltering the other), one
seed each, `cinematic` preset leading every prompt. Dead-flat fraction at native size:

| | set A handshake | set B shoulder | set C shelter |
|---|---|---|---|
| HiDream dev **+ mocap skeleton** | 44.3 % | 46.9 % | 56.0 % |
| HiDream dev, text only | 74.6 % | 79.0 % | 81.6 % |
| FLUX.2-dev turbo | **4.9 %** | **7.9 %** | 21.0 % |
| Ideogram 4, JSON caption | 27.2 % | 39.6 % | 29.0 % |
| HiDream full + romsketch | 52.4 % | 52.2 % | 53.2 % |

Reading them rather than the table:

* **HiDream text-only is the floor**, and consistently — 74-82 % of the frame carries no texture at
  all. It is not a bad-seed problem.
* **The mocap-conditioned lane pays for its pose.** It is 20-30 points better than text-only
  HiDream, which is the skeleton doing work, but the figures come out small in frame and at frame 0
  the *interaction* is not readable — you can see two people, not two people shaking hands.
* **Ideogram drew four people in set A**, against a caption and a layout box that both said two.
  That is the second subject-count miss in two sittings (three-for-two in the four-model set). It
  is the best texture-per-second lane and it cannot be trusted to count.
* **romsketch is the consistency win**: 52.4 / 52.2 / 53.2 %, all three photographic, all three with
  the right number of people. A three-tenths-of-a-point spread across three different scenes is not
  something the other four lanes came close to.
* **FLUX.2 has the best texture by a wide margin** — and in all three sets it was wearing a black
  oval mask.

### The oval was mine, and it is now measured and gone

Not a FLUX.2 quirk. The `cinematic` preset I rewrote earlier in this session opens

> anamorphic widescreen, **oval highlight bokeh** and shallow focus, ...

and FLUX.2 renders "oval" as a shape to draw: a hard black field with a soft-edged oval cut out of
the middle, like looking down a telescope. Measured as mean brightness of the four corner sixths
over the centre sixth, where natural vignetting runs 0.7-0.9:

```
as written (oval highlight bokeh) : corners   3.2  centre 58.0  ratio 0.05  dead-flat 21.0 %
that clause deleted               : corners  60.5  centre 29.3  ratio 2.06  dead-flat 11.7 %
shipped wording                   : corners  36.0  centre 38.7  ratio 0.93  dead-flat  8.7 %
```

Same seed, same subject, same everything else. The clause is worth two thirds of the frame.

The fix is one line in `sequences/styles.py`: "oval highlight bokeh and shallow focus" becomes "a
long lens held wide open so the background falls softly away". Ratio 0.93 is inside the natural
band, and the frame is the *best-textured* of the three at 8.7 % dead-flat — deleting the clause
outright over-brightened the corners (2.06) where the replacement does not.

What makes this worth more than a one-line diff: the comment block directly above that preset
already says a style clause "must not name anything the picture could contain", and lists two cases
caught the same way. It was written about nouns. **An adjective naming a shape is a noun to the
model** — even when the shape belongs to the lens and not the scene. That case is now the third
entry in that list, with the numbers, so the next rewrite has it.

### Why the terminal died twice

The operator's terminal was OOM-killed twice during the image runs. `journalctl -k` has three
`oom-kill` events, all naming the wezterm scope — systemd kills per-scope, so the terminal dies
while the process that actually ate the memory survives. 31 GB RAM, 3.7 GB swap, and swap was at
100 % both times.

The cause is additive, not any single hog: ComfyUI with FLUX.2 or Ideogram loaded offloads ~8 GB to
*system* RAM (not just VRAM), musubi with `--blocks_to_swap 24` does the same, and `uv run pytest`
forks a worker per core. Two of those three together fit; three do not.

So, as a rule for this box: **stop the GPU tenant before switching models** (`content-factory
services stop` — it reported `comfyui stopped` and gave 2 GB back before the gate run below),
**run renders in the foreground, one at a time**, and **never run the test suite while a model is
resident**. The `free -g` "available" column is the one to read, not "free": 25 GB available with
swap still showing 3/3 is fine, because the swap pages are stale.

### Gates

All six, after the preset fix, with no GPU tenant running:

```
just fmt          638 files left unchanged
just lint         0 errors (warnings only, all pre-existing jsx-a11y/react)
just typecheck    0 errors, 0 warnings   (fixed one: tests/unit/test_anchor_conditioning.py:104
                                          read .joints off a tuple[SkeletonPose | None, ...] entry)
uv run pytest     1530 passed, 2 skipped, 74 deselected in 352.02s
pnpm -r test      463 passed across 9 packages
just test-integration   68 passed, 1538 deselected in 272.12s
```

`test-integration` was required here and not optional: this session rewrote `workflows/stages.py`
around anchor rejection, redirects and seed offsets, and the core suite cannot reach
`ProductionWorkflow`.
