...
docker exec content-factory-postgres-1 psql -U content_factory -d content_factory -tc \
  "SELECT 1 FROM pg_database WHERE datname='cf_gate'" | grep -q 1 || \
  docker exec content-factory-postgres-1 psql -U content_factory -d content_factory -c "CREATE DATABASE cf_gate" >/dev/null
docker exec content-factory-postgres-1 psql -U content_factory -d content_factory -tc \
  "SELECT 1 FROM pg_database WHERE datname='cf_gate_test'" | grep -q 1 || \
  docker exec content-factory-postgres-1 psql -U content_factory -d content_factory -c "CREATE DATABASE cf_gate_test" >/dev/null
cp .env.example .env
...
uv run pytest -q -m "not integration and not gpu and not live" 2>&1 | tail -1
