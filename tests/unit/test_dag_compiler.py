...
AUDIO_VIDEO = {
    Stage.lock_script,
    Stage.synthesize_narration,
    Stage.align_words,
    Stage.compile_captions,
    Stage.mix_audio,
    Stage.compile_timeline,
    Stage.render_scenes,
    Stage.compose_video,
}
...
def test_image_only_job_invokes_no_audio_or_video_stages() -> None:
    dag = compile_dag(
        _campaign(content.SingleImagePostSpec(deliverable_id="dlv_image0000001", title="card"))
    )
    assert dag.stages().isdisjoint(AUDIO_VIDEO)
    assert {
        Stage.compile_artboards,
        Stage.render_static,
...
def test_short_video_gets_audio_then_video_in_order_and_shared_stages_once() -> None:
    ...
    assert (
        order.index("synthesize_narration:dlv_short0000001")
        < order.index("align_words:dlv_short0000001")
        < order.index("compile_timeline:dlv_short0000001")
        < order.index("compose_video:dlv_short0000001")
    )
    ...
    dag = compile_dag(
        _campaign(
            content.ShortVideoSpec(
                deliverable_id="dlv_short0000002", title="silent", narration=False, music=False
            )
        )
    )
    assert Stage.synthesize_narration not in dag.stages()
    assert any(
        p.stage == Stage.synthesize_narration and p.deliverable_id == "dlv_short0000002"
        for p in dag.pruned
    )
...
def test_approved_copy_transform_prunes_research() -> None:
    brief = sample_brief().model_copy(
        update={
            "input_mode": content.InputMode.approved_copy_transform,
            "pasted_copy": "Approved text.",
        }
    )
    camp = content.ContentCampaign(
        campaign_id="cmp_test00000002",
        workspace_id=WS,
        brief=brief,
        deliverables=(content.TextPostSpec(deliverable_id="dlv_text00000001", title="t"),),
    )
