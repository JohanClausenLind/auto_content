...
def test_searxng_adapter_parses_json_results() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            ...
            json={
                "results": [
                    {
                        "title": "Energy in Sweden",
                        "url": "https://example.se/r",
                        "content": "wind 21%",
                        "engine": "duckduckgo",
                    },
    ...
    provider = SearxngSearchProvider(
        "http://127.0.0.1:8083", transport=httpx.MockTransport(handler)
    )
...
def test_article_extraction_from_fixture_html() -> None:
    ...
    cap = CapturedContent(
        "https://example.se/wind", "https://example.se/wind", 200, "text/html", html, "0" * 64, ()
    )
    doc = extract(cap)
