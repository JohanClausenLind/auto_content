...
def token_transport(seen: list[dict]) -> httpx.MockTransport:
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if "code_verifier" in body:
            ...
            expected_challenge = base64.urlsafe_b64encode(hashlib.sha256(unquote(body["code_verifier"]).encode()).digest()).rstrip(b"=").decode()
        return httpx.Response(200, json={"access_token": "access-tok-1", "refresh_token": "refresh-tok-1", "expires_in": 3600, "_challenge": expected_challenge})
...
async def _ws(sessionmaker, ws_id: str, slug: str) -> None:
    ...
    async with sessionmaker() as db:
        await db.execute(insert(Workspace).values(id=ws_id, slug=slug, name=slug, settings={}).on_conflict_do_nothing(index_elements=[Workspace.id]))
        await db.commit()
...
async def test_full_flow_seals_tokens_and_verifies_pkce(sessionmaker, vault) -> None:
    ...
    seen: list[dict] = []
    async with sessionmaker() as db:
        url, state = await begin(db, workspace_id=WS_A, app=APP, redirect_uri="http://127.0.0.1:3000/callback")
        await db.commit()
    assert "code_challenge=" in url and "code_challenge_method=S256" in url and f"state={state}" in url
    async with sessionmaker() as db:
        account = await complete(db, workspace_id=WS_A, app=APP, state=state, code="authcode", vault=vault, handle="op@masto.example", transport=token_transport(seen))
        await db.commit()
    body = seen[0]
    ...
    assert "access-tok-1" not in json.dumps({"ct": account.token_ciphertext, "n": account.token_nonce})
...
async def test_replay_foreign_state_expiry_and_workspace_binding(sessionmaker, vault) -> None:
    ...
    seen: list[dict] = []
    async with sessionmaker() as db:
        _url, state = await begin(db, workspace_id=WS_A, app=APP, redirect_uri="http://127.0.0.1:3000/cb")
        await db.commit()
    # Wrong workspace
    async with sessionmaker() as db:
        with pytest.raises(OAuthError, match="different workspace"):
            await complete(db, workspace_id="ws_other0000001", app=APP, state=state, code="c", vault=vault, handle="h", transport=token_transport(seen))
        await db.rollback()
    # Consume once, then replay is refused.
    async with sessionmaker() as db:
        await complete(db, workspace_id=WS_A, app=APP, state=state, code="c", vault=vault, handle="h", transport=token_transport(seen))
        await db.commit()
    async with sessionmaker() as db:
    ...
    async with sessionmaker() as db:
        with pytest.raises(OAuthError, match="unknown state"):
            await complete(db, workspace_id=WS_A, app=APP, state="forged", code="c", vault=vault, handle="h", transport=token_transport(seen))
        await db.rollback()
